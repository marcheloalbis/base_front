...
import { MenuUsuarioService } from 'src/app/services/menu.service';
...
export class AppMenuComponent implements OnInit {
    ...
    constructor(public layoutService: LayoutService, private sessionService: SessionService, private menuService: MenuUsuarioService) { }

    parseModel (array: any[]) {
        let response: any [] = [];
        array.forEach(elem => {
            
            // let obj = { };
            let obj = { label: elem.name, icon: 'pi pi-desktop', routerLink: ['/'+ elem.route] };
            response.push(obj);
        });
    ...
    ngOnInit() {
        ...
        if (this.targetUser.ide) {
            this.menuService.getMenu(this.targetUser.ide).subscribe ( data=> {
                console.log('MENU: ', JSON.stringify(data, null, 4));
...
/*         this.model = [
            {
                label: 'Dashboard', icon: 'pi pi-home',
                items: [
                    { label: 'Main', icon: 'pi pi-desktop', routerLink: ['/home'] },
                    { label: 'Usuario', icon: 'pi pi-user', routerLink: ['/user'] },
                    { label: 'Rol', icon: 'pi pi-user', routerLink: ['/rol'] },
                    { label: 'Opcion', icon: 'pi pi-user', routerLink: ['/opcion'] },
                ]
            }, */
